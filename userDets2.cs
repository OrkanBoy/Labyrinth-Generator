...
namespace LearningCSharpPart1
{
    ...
    public class Room
    {
        ...
        //to prevent reprocessing the samething multiple times for the contnuos variant rich generation of the inifnte maze
        public bool candidateForGeneration;
        public Room(bool northernRoomOpening, bool easternRoomOpening, bool southernRoomOpening, bool westernRoomOpening, bool playerOccupationRoom, bool roomBeenVisited)
        {
            ...
            //only modified manually with Room.candidateForGeneration=false/true;
            candidateForGeneration = true;
    ...
    public class ifDFSShouldEnd
    {
        public bool continueAllowance;
        public short generationRegularity;
        
        public ifDFSShouldEnd( short generationStability)
        {
            generationRegularity = generationStability;
            if (generationStability != 1 || generationRegularity  !=-1)
            {
                continueAllowance = true;
            }
            else
            {
                continueAllowance = false;
            }
        }
        public void update()
        {
            if (generationRegularity == 1)
            {
                continueAllowance = false;
            }
            else
